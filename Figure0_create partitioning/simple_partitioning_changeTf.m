...
% community while changing the amount of time the simulation is run. 
...
tends = [50 100 200]; % the length of simulation
...
for i = 1:length(tends)
    tic
    yend_house = runSeg_2gamma(params, y0_, nonEmpty, cell_desensity_max, tends(i));
    toc
    ...
    figure(1)
    hold on
    plot(N_,BIsimpson,'o-','color',[0.1,0.1,0.1]*2*i)
    set(gca,'xscale','log')

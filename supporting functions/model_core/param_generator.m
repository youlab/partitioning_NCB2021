...
%%% Hard code del for 2 species -- 2bcd additional plots
% del = [1.1; 0.1]; 
...
%%% Hard code lower part to 0 -- 2bcd additional plots
% onesM = ones(2,2);
% idx = tril(onesM,-1) ; % Make 1's at lower triangular part 
% display(idx);
%%% use beta_f(idx~=1) = 0 if only positive; use gam_f(idx~=1) = 0 only neg
% beta_f(idx~=1) = 0;  % beta ~= creates first helps second
% gam_f(idx~=1) = 0; % neg
